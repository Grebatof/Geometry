...
void error_handling(int *i, int size, char * arr, int error, int row_count) {
  ...
  switch (error) {
    ...
    break;
    case 15:
    printf("\n(row: %d) Wrong last/first coordinates (first coordinates must be equal last coordinates)\n\n\n", row_count);
    break;
    case 16:
    printf("\n(row: %d) Missing ')'\n\n\n", row_count);
    break;
...
int figure_check(char * figure_name) {
  ...
  if (!(strcmp(figure_name, "circle"))) return 1;
  if (!(strcmp(figure_name, "triangle"))) return 2;
  if (!(strcmp(figure_name, "poligon"))) return 3;
...
int processing_radius(int *i, int size, char * arr, Geometry *geo) {
  ...
  for (; *i < size; (*i)++) {
    ...
    if (isdigit(arr[*i])) {
      geo->circle[geo->size_circle - 1].radius = (float) (arr[*i] - '0') / (pow(10, powka)) + geo->circle[geo->size_circle - 1].radius; // преобразуем строку в число
      numbers_points++;
      powka++;
      continue;
    }
...
int processing_coordinates_circle(int *i, int size, char * arr, Geometry *geo) {
  ...
  int symbol_minus = 0;
  int numbers_points = 0;
  geo->circle[geo->size_circle - 1].point.x = geo->circle[geo->size_circle - 1].point.y = geo->circle[geo->size_circle - 1].radius = 0; // "зануляем" значения // можно сделать функцию zeroing();

  if (arr[(*i)] != '(') return 11;
  (*i)++;
  for (; *i < size; (*i)++) {
    if (symbol_minus > 1) return 12; // проверка на кол-во минусов (если больше 1 - возвращение места, где произошла ошибка)
    ...
    if (isdigit(arr[*i])) {
      geo->circle[geo->size_circle - 1].point.x = arr[*i] - '0' + (geo->circle[geo->size_circle - 1].point.x * 10); // преобразуем строку в число
      numbers_points++;
      continue;
    }
  ...
  for (; *i < size; (*i)++) {
    ...
    if (isdigit(arr[*i])) {
      geo->circle[geo->size_circle - 1].point.y = arr[*i] - '0' + (geo->circle[geo->size_circle - 1].point.y * 10); // преобразуем строку в число
      numbers_points++;
      continue;
    }
    if (arr[*i] == ',') {
...
int processing_first_coordinates_triangle(int *i, int size, char * arr, Geometry *geo) {
  ...
  for (; *i < size; (*i)++) {
    ...
    if (isdigit(arr[*i])) {
      geo->triangle[geo->size_triangle - 1].point[0].x = arr[*i] - '0' + (geo->triangle[geo->size_triangle - 1].point[0].x * 10); // преобразуем строку в число
      numbers_points++;
      continue;
    }
    ...
    if (arr[*i] == ' ') {
      if (numbers_points > 0) {
        if (symbol_minus == 1) geo->triangle[geo->size_triangle - 1].point[0].x *= -1;
        break;
      }
      return 17;
    } else {
  ...
  for (; *i < size; (*i)++) {
    ...
    if (isdigit(arr[*i])) {
      geo->triangle[geo->size_triangle - 1].point[0].y = arr[*i] - '0' + (geo->triangle[geo->size_triangle - 1].point[0].y * 10); // преобразуем строку в число
      numbers_points++;
      continue;
    }
    if (arr[*i] == ',') {
      if (numbers_points > 0) {
        if (symbol_minus == 1) geo->triangle[geo->size_triangle - 1].point[0].y *= (-1);
        break;
      }
      return 18;
    } else {
...
int processing_next_coordinates_triangle(int *i, int size, char * arr, Geometry *geo) {
  ...
  for (; *i < size; (*i)++) {
    ...
    if (isdigit(arr[*i]) && arr[*i - 1] == ',') return 21;
    if (space > 2) return 19;
    if (symbol_minus > 1) return 12;
    /*if (point_number) {
      counterXY -= 2;
    }*/
    if (arr[*i] == '-' && numbers_points == 0) {
    ...
    if (counterXY == 0 && isdigit(arr[*i])) {
      geo->triangle[geo->size_triangle - 1].point[point_number].x = arr[*i] - '0' + (geo->triangle[geo->size_triangle - 1].point[point_number].x * 10);
      numbers_points++;
      countX++;
      continue;
    }
    ...
    if (counterXY == 1 && isdigit(arr[*i]) && countX > 0) {
      geo->triangle[geo->size_triangle - 1].point[point_number].y = arr[*i] - '0' + (geo->triangle[geo->size_triangle - 1].point[point_number].y * 10);
      numbers_points++;
      countY++;
      continue;
    } else if (countX == 0) {
...
int processing_first_coordinates_poligon(int *i, int size, char * arr, Geometry *geo) {
  ...
  for (; *i < size; (*i)++) {
    ...
    if (isdigit(arr[*i])) {
      geo->poligon[geo->size_poligon - 1].points[0].x = arr[*i] - '0' + (geo->poligon[geo->size_poligon - 1].points[0].x * 10); // преобразуем строку в число
      numbers_points++;
      continue;
    }
    ...
    if (arr[*i] == ' ') {
      if (numbers_points > 0) {
        if (symbol_minus == 1) geo->poligon[geo->size_poligon - 1].points[0].x *= -1;
        break;
      }
      return 17;
    } else {
  ...
  for (; *i < size; (*i)++) {
    ...
    if (isdigit(arr[*i])) {
      geo->poligon[geo->size_poligon - 1].points[0].y = arr[*i] - '0' + (geo->poligon[geo->size_poligon - 1].points[0].y * 10); // преобразуем строку в число
      numbers_points++;
      continue;
    }
    if (arr[*i] == ',') {
      if (numbers_points > 0) {
        if (symbol_minus == 1) geo->poligon[geo->size_poligon - 1].points[0].y *= (-1);
        break;
      }
      return 18;
    } else {
...
int processing_next_coordinates_poligon(int *i, int size, char * arr, Geometry *geo) {
  ...
  for (; *i < size; (*i)++) {
    ...
    if (counterXY == 0 && isdigit(arr[*i])) {
      geo->poligon[geo->size_poligon - 1].points[point_number].x = arr[*i] - '0' + (geo->poligon[geo->size_poligon - 1].points[point_number].x * 10);
      numbers_points++;
      countX++;
      continue;
    }
    ...
    if (counterXY == 1 && isdigit(arr[*i]) && countX > 0) {
      geo->poligon[geo->size_poligon - 1].points[point_number].y = arr[*i] - '0' + (geo->poligon[geo->size_poligon - 1].points[point_number].y * 10);
      numbers_points++;
      countY++;
      continue;
    } else if (countX == 0) {
